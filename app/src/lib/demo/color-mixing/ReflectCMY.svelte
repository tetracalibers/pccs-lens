<script lang="ts">
  ...
  const GAP_BAR_TO_LABEL = 50 // バー右端から C/M/Y ラベル中心までの距離
  const LABEL_X = BAR_X + BAR_WIDTH + GAP_BAR_TO_LABEL // C/M/Y ラベルの x 中心（右側、導出値）
   ...
   * CMY は RGB と反転しており、反射領域が多数派になる。
   * - C (シアン)   : 左2/3 を反射（短波長 + 中波長）、右1/3 を吸収
   * - M (マゼンタ) : 左1/3 と右1/3 を反射（短波長 + 長波長）、中1/3 を吸収
   * - Y (イエロー) : 左1/3 を吸収、右2/3 を反射（中波長 + 長波長）
   */
  const rows: Row[] = [
    {
      label: "C",
  ...
  /**
   * バーローカル座標での「反射」クリップ矩形の配列を返す。
   * M のように非連続な反射領域（左1/3 + 右1/3）にも対応する。
   */
  function reflectClipRects(row: Row): ClipRect[] {
    return resolveRegions(row)
...
>
  <defs>
    <!-- 可視光スペクトルグラデーション -->
    <linearGradient id="rcmy-spectrum" x1="0" y1="0" x2="1" y2="0">
    ...
    <!-- 各行の反射クリップパス（バーのローカル座標で定義、M は rect が 2 つ） -->
    {#each rows as row (row.label)}
      <clipPath id="rcmy-clip-{row.label}">
        {#each reflectClipRects(row) as cr (cr.x)}
    ...
    <marker id="rcmy-aL-absorb" markerWidth="9" markerHeight="10" refX="1" refY="5" orient="auto">
      <polyline
        points="8,1 1,5 8,9"
        fill="none"
        stroke={COLOR_ABSORB}
    ...
    <marker id="rcmy-aR-absorb" markerWidth="9" markerHeight="10" refX="8" refY="5" orient="auto">
      <polyline
        points="1,1 8,5 1,9"
        fill="none"
        stroke={COLOR_ABSORB}
    ...
    <marker id="rcmy-aL-reflect" markerWidth="9" markerHeight="10" refX="1" refY="5" orient="auto">
      <polyline
        points="8,1 1,5 8,9"
        fill="none"
    ...
    <marker id="rcmy-aR-reflect" markerWidth="9" markerHeight="10" refX="8" refY="5" orient="auto">
      <polyline
        points="1,1 8,5 1,9"
        fill="none"
        stroke={COLOR_REFLECT}
  ...
  {#each rows as row, i (i)}
    ...
    {#each regions as region (region.absX)}
      ...
      <line
        ...
        stroke={color}
        stroke-width="2"
        marker-start="url(#rcmy-aL-{markerId})"
        marker-end="url(#rcmy-aR-{markerId})"
      />
    {/each}
    ...
    <g transform="translate({BAR_X}, {bTop})">
      ...
      <g clip-path="url(#rcmy-clip-{row.label})">
        <rect x="0" y="0" width={BAR_WIDTH} height={BAR_HEIGHT} fill="url(#rcmy-spectrum)" />
      </g>
      <rect
        x="0"
        y="0"
