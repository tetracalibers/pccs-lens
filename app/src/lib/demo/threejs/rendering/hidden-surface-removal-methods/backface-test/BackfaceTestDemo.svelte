...
<ThreeDemoCanvas
  ariaLabel="半透明の立方体と、そこから離して置いた1つの視点の3次元表示。手前の1面にだけ、面に垂直な法線ベクトルと、面から視点へ向かう視点方向ベクトル、その2つがなす角を示す扇を添えている。視点の方位を動かすと扇の広がりが直角をまたいで変わり、鈍角になった面では視点方向ベクトルが立方体の内部を貫く。既定では裏を向いた面を取り除いた状態で、裏面を外すを切り替えると、裏を向いた面が暗い色で戻る（ドラッグで回転、ホイールで拡大縮小）"
