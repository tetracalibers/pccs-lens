...
<ThreeDemoCanvas
  ariaLabel="4つの制御点から作った3次ベジェ曲線の図。制御点を順に結んだ制御多角形を破線と、その内側を薄く塗った面で描き、接ベクトルの向きを決める最初の辺と最後の辺だけを実線で重ねている。両端の制御点からは、その辺と同じ向きに辺の3倍の長さで伸びる接ベクトルを矢印で描いている。制御点はドラッグで動かすことができ、動かすと曲線と接ベクトルが引き直される（ホイールで拡大縮小）"
