...
<ThreeDemoCanvas
  ariaLabel="左に、投影面に面を接した立方体と、その投影面。立方体の各頂点から投影面へ斜めに引いた投射線と、投影面に写った像が描かれる。右に、その像を正面から見たもので、像の上の3つの座標軸と、縮まなかったときの軸の先を示す円が描かれる。奥行きの倍率と向きを変えると、像の上で奥行きの辺の長さと向きが変わる（ホイールで拡大縮小、回転はしない）"
  createScene={createObliqueScene}
  {params}
  aspectRatio="16 / 9"
  camera={{ position: [0, 0, 5.6] }}
  orbit={{
    // 像を正面から読み取る図なので回転させない（奥行きの辺の長さと向きが正しく見えなくなる）。
