<script lang="ts">
  ...
  function smoothCurveSegments(pts: ReflectancePoint[]): string {
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {TOTAL_WIDTH} {TOTAL_HEIGHT}">
  <defs>
    ...
    <clipPath id="reflectance-plot-clip-{id}">
  ...
  <path
    d={fillPath}
    fill="url(#reflectance-gradient-{id})"
    clip-path="url(#reflectance-plot-clip-{id})"
  />
