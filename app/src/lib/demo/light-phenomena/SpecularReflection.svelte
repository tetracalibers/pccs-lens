<script lang="ts">
  ...
  const NORMAL_LEN = 170 // 法線（点線）の長さ
  ...
  const normalTop: Pt = { x: 0, y: -NORMAL_LEN }

  // 光線の端点
  const inSource: Pt = { x: -RAY_LEN * S, y: -RAY_LEN * C } // 入射光の始点（左上）
  const reflEnd: Pt = { x: RAY_LEN * S, y: -RAY_LEN * C } // 反射光の終点（右上）

  // 矢じりを置く中間点
  const inMid: Pt = { x: -ARROW_FRAC * RAY_LEN * S, y: -ARROW_FRAC * RAY_LEN * C }
  const reflMid: Pt = { x: ARROW_FRAC * RAY_LEN * S, y: -ARROW_FRAC * RAY_LEN * C }

  // 扇形の弧の端点
  const arcNormal: Pt = { x: 0, y: -ARC_R }
  const arcIn: Pt = { x: -ARC_R * S, y: -ARC_R * C }
  const arcRefl: Pt = { x: ARC_R * S, y: -ARC_R * C }
  ...
  const VB_Y = -(NORMAL_LEN + PAD)
  const VB_W = 2 * (GROUND_HALF + PAD)
  const VB_H = NORMAL_LEN + 2 * PAD
