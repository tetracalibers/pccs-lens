<script lang="ts">
  ...
  const COL_PHOTOPIC = PCCS_HEX_MAP.get("lt2")! // 明所視（赤）
  const COL_SCOTOPIC = PCCS_HEX_MAP.get("lt18")! // 暗所視（青）
  ...
  const PHOTOPIC_PEAK_NM = 555

  // ===== 凡例（プロット内右上） =====
  const LEGEND_LINE_LENGTH = 56
  const LEGEND_ROW_GAP = 32
  const LEGEND_LABEL_GAP = 14
  const LEGEND_TEXT_WIDTH = FONT_SIZE_LEGEND * 3.0 // 「明所視」「暗所視」(3 全角文字) のおおよその幅
  ...
  // 枠の位置（右上アンカー）
  const LEGEND_FRAME_X = PLOT_RIGHT - 174 - LEGEND_FRAME_PADDING_X
  ...
  const LEGEND_Y_PHOTOPIC = LEGEND_FRAME_Y + LEGEND_FRAME_PADDING_Y
  const LEGEND_Y_SCOTOPIC = LEGEND_Y_PHOTOPIC + LEGEND_ROW_GAP
  ...
  const PHOTOPIC_DATA: EfficiencyPoint[] = [
  ...
  const SCOTOPIC_DATA: EfficiencyPoint[] = [
  ...
  const photopic: EfficiencyPoint[] = sampleWavelengths.map((nm) => ({
    nm,
    value: interpolate(PHOTOPIC_DATA, nm)
  }))
  const scotopic: EfficiencyPoint[] = sampleWavelengths.map((nm) => ({
    nm,
    value: interpolate(SCOTOPIC_DATA, nm)
  }))
  ...
  const photopicPath = lineGen(photopic) ?? ""
  const scotopicPath = lineGen(scotopic) ?? ""
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {TOTAL_WIDTH} {TOTAL_HEIGHT}">
  <defs>
    <marker
      id="photopic-scotopic-arrow-photopic"
    ...
    <marker
      id="photopic-scotopic-arrow-scotopic"
      viewBox="0 0 {ARROW_HEAD_VIEWBOX} {ARROW_HEAD_VIEWBOX}"
      refX={ARROW_HEAD_VIEWBOX / 2}
      refY={ARROW_HEAD_VIEWBOX / 2}
      markerWidth={ARROW_HEAD_SIZE}
    ...
    <clipPath id="photopic-scotopic-plot-clip">
      <rect x={PLOT_LEFT} y={PLOT_TOP} width={PLOT_WIDTH} height={PLOT_HEIGHT} />
    </clipPath>
    <linearGradient id="photopic-scotopic-spectrum-gradient" x1="0" y1="0" x2="1" y2="0">
  ...
  <rect
    ...
    fill="url(#photopic-scotopic-spectrum-gradient)"
  />
  ...
  <path
    ...
    stroke-linejoin="round"
    stroke-linecap="round"
    clip-path="url(#photopic-scotopic-plot-clip)"
  />
  ...
  <!-- 暗所視のピーク (507nm)：X軸下、上向き矢印 + 2行ラベル -->
  <line
    x1={xAt(SCOTOPIC_PEAK_NM)}
    y1={X_ANNO_ARROW_TAIL_Y}
    x2={xAt(SCOTOPIC_PEAK_NM)}
    y2={X_ANNO_ARROW_TIP_Y}
    stroke={COL_SCOTOPIC}
    stroke-width={STROKE_WIDTH_ARROW}
    stroke-linecap="round"
    marker-end="url(#photopic-scotopic-arrow-scotopic)"
  />
  <text
  ...
  <!-- 明所視のピーク (555nm)：X軸下、上向き矢印 + 2行ラベル -->
  <line
    x1={xAt(PHOTOPIC_PEAK_NM)}
    y1={X_ANNO_ARROW_TAIL_Y}
    x2={xAt(PHOTOPIC_PEAK_NM)}
    y2={X_ANNO_ARROW_TIP_Y}
    stroke={COL_PHOTOPIC}
    stroke-width={STROKE_WIDTH_ARROW}
    stroke-linecap="round"
    marker-end="url(#photopic-scotopic-arrow-photopic)"
  />
  <text
  ...
  <g>
    ...
    <line
      x1={LEGEND_LINE_X1}
      y1={LEGEND_Y_PHOTOPIC}
      x2={LEGEND_LINE_X2}
      y2={LEGEND_Y_PHOTOPIC}
      stroke={COL_PHOTOPIC}
      stroke-width={STROKE_WIDTH_CURVE}
      stroke-linecap="round"
    />
    ...
    <line
      x1={LEGEND_LINE_X1}
      y1={LEGEND_Y_SCOTOPIC}
      x2={LEGEND_LINE_X2}
      y2={LEGEND_Y_SCOTOPIC}
      stroke={COL_SCOTOPIC}
      stroke-width={STROKE_WIDTH_CURVE}
      stroke-linecap="round"
    />
