<script lang="ts">
  ...
  // ===== 無彩色の明るさ範囲（0–255 のグレイ値。外＝明、内＝暗） =====
  const GRAY_MIN = 40 // 最も内側の正方形の明るさ
  const GRAY_MAX = 235 // 最も外側の正方形の明るさ
  ...
  // 外側（明）から内側（暗）へ均等に暗くなる正方形。
  // i=0 が最も外側で最大・最明、i=STEP_COUNT-1 が最も内側で最小・最暗。
  ...
  const SQUARES = Array.from({ length: STEP_COUNT }, (_, i) => {
    ...
    const gray = grayHex(GRAY_MAX - ((GRAY_MAX - GRAY_MIN) * i) / (STEP_COUNT - 1))
    return { size, offset, gray }
  })
</script>

<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {WIDTH} {HEIGHT}">
  <!-- 中心を揃えて重ねた正方形。外側ほど明るく、内側ほど暗い。
       各正方形の縁が交わる対角線上で縁辺対比が強まり、
       角から中心へ向かう暗い対角線（ヴァザレリ錯視）が見える -->
