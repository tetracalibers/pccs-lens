<script lang="ts">
  ...
  // ===== 線・ギャップ =====
  const STROKE_WIDTH_GRID = 3 // 白い格子線の太さ
  const GAP_RADIUS = 16 // 十字路の抜けた部分（黒い円）の半径
  const STROKE_WIDTH_RING = 3 // 抜けた部分を囲む実線の太さ

  // ===== 色 =====
  const COL_GRID = "#ffffff" // 白い格子線
  const COL_BG = "#000000" // 黒背景・抜けた部分
  const COL_RING = "#ffffff" // 抜けた部分を囲む実線
