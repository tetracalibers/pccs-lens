...
- `as`の利用を避ける。必要に応じてユーティリティ型を実装する

### Svelte

- Svelte 5 の Runes（`$state`、`$derived`、`$effect` 等）を使用する
- ページ固有のロジックが複雑になる場合は `src/libs`配下の`.ts`ファイルに切り出す
- コンポーネントのpropsは `$props()` で型付きに定義する

### 関数・モジュール

- `src/lib/`配下の`.ts`ファイルのロジックはUIに依存しない純粋関数として実装する
...
- コンポーネントのスタイルは各 `.svelte` ファイルの `<style>` ブロックに記述する（scoped CSS）
...
- アプリケーションのページで使用される`src/lib/`配下のすべての関数にユニットテストを必須とする
