...
// これがないと、同じユニットのページが必ずしも近くに来ないので、かたまりとして読めなくなる。
//
 ...
 *
 * `unit` が null のノード（ゴースト・リンク切れ・所属不明）はどのユニットにも寄せないので、
 * どちらの対象にもしない。
 */
...
export const createSimulation = ({ onTick, onSettle, labelWidth }) => {
  ...
  const sync = ({ nodes: input, links: inputLinks, unitOrder, pin }) => {
    ...
    /** ユニットに属さないノードの置き場所。リンク相手の平均に寄せ、相手がいなければ外周へ逃がす。 */
