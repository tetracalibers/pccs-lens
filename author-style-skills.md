...
- **引数なしの場合** … 解説記事（`layout: guide-content`）の `.svx` をすべて参照する。`concept`（`layout: concept`）など記事以外のページは除外する。対象が多くトークン消費が大きいため、**分析前に必ずユーザーへ大量消費の恐れを伝え、続行してよいか確認する**
...
リード（Coordinator）は `TaskCreate` で共有タスクリストを用意し、`Agent` ツールで役割ごとに teammate を spawn します（`name` 必須・`run_in_background: true`・`subagent_type` は `general-purpose`）。spawn プロンプトには担当役割／分析対象／分析プロンプトと出力契約のパス／他 teammate の `name` 一覧／共有タスクリストの参照方法／成果物の出力先を渡します。最終成果物は `SendMessage` ではなく `writing-guides/` 配下のファイルへの書き込みで受け渡し、監査後にリードがファイルの作成・更新を検証してユーザーへ報告します。起動手順と受け渡しの詳細は `collaboration-workflow.md` の「実行方式（Agent Teams）」を参照。
...
作業は次の9フェーズを順に進めます。独立分析とレビューを同時に行いません。
...
9. リードによる回収と報告（成果物ファイルの作成・更新を検証し、ユーザーへ変更内容を報告。待機状態で残る teammate は `TaskStop` で明示停止）
