...
対称性と描き方が違う 3 つの版がある。構図・配色のルールは共通。
...
node src/generate.js --help          # オプション一覧（--help は 3 つのスクリプトで共通）
```

Output goes to `.generated/{curved|angular|wallpaper}/{YYYYMMDD-THHMMSSZ}/` (SVG + `index.html` の一覧ページ).
...
- `src/composition.js` — 帯の分割・配色・アクセント色といった、3 つの版で共通の構図ルール
- `src/motif.js` / `src/motif-angular.js` / `src/motif-wallpaper.js` — 基本領域 1 枚ぶんの模様の組み立て（ここだけが版ごとに違う）
- `src/render.js` — ロゼッタの SVG 組み立てと、3 版で共通の一覧 HTML
- `src/render-wallpaper.js` — 壁紙群の SVG 組み立て（基本領域 → 群の変換 → 平行移動の 3 段構え）とガイドの描画
- `src/cli-shared.js` — 引数解釈・シード・出力先・後始末（3 版で共通）
