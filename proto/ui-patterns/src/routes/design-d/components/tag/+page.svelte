  ...
  const patterns = [
    {
      id: "a",
      label: "A — パステルソリッド",
      note: "パステルカラーの塗り背景に濃いテキスト。グレードごとに異なるパステルトーンを割り当て、ポップで明るい印象を与える。「入門」は暖色のコーラルで際立たせる"
    },
    {
      id: "b",
      label: "B — ビビッドアウトライン",
      note: "彩度の高い輪郭線と薄い背景色の組み合わせ。「入門」は太い枠線＋アイコン付きで存在感を強調。他グレードは細い枠線でおとなしく並ぶ"
    },
    {
      id: "c",
      label: "C — キャンディピル",
      note: "pill形状にパステルの塗り背景を組み合わせたキャンディ風スタイル。「入門」はグラデーション＋白文字で他グレードと質感を変えて際立たせる"
    },
    {
      id: "d",
      label: "D — ドットアクセント",
      note: "カラードットを先頭に置いたミニマルなラベル。「入門」はドットのサイズを大きくしてリング付きのパルスアニメーションを加え、視線を引き込む"
    },
    {
      id: "e",
      label: "E — グラデーションバッジ",
      note: "2色グラデーションの背景バッジ。他グレードはパステル同士の淡い組み合わせにする一方、「入門」はウォームサンセット（珊瑚→橙）で温かみを強調する"
    },
    {
      id: "f",
      label: "F — ネオングロー",
      note: "テキストと枠にグロー効果を乗せたダーク映えスタイル。「入門」はピンクの強い発光＋CSSアニメーションで点滅する光輪を加え、唯一の動的な表現にする"
    },
    {
      id: "g",
      label: "G — アンダーライン装飾",
      note: "カラーのボトムボーダーのみのシンプルなラベル。「入門」はレインボーグラデーションの下線と太字で差別化。他グレードは単色の細い下線で統一する"
    },
    {
      id: "h",
      label: "H — ブラケットスタイル",
      note: "コード・技術文書を想起させる括弧スタイル。「入門」は全角の日本語括弧【】と暖色で視覚的に浮かせる。他グレードは半角の [ ] でモノスペース的に統一する"
    }
  ]
</script>
...
<style>
  ...
  .tag.-g[data-grade="intro"] {
    ...
    font-size: 0.82rem;
    padding-bottom: 4px;
    background: linear-gradient(90deg, #f43f5e, #fb923c, #fcd34d) no-repeat bottom / 100% 2.5px;
  }
