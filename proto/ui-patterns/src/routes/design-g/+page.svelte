<script lang="ts">
  ...
  const contentLinks = [
    ...
    { href: "#", title: "PCCSのトーン", desc: "明度・彩度をまとめた17のトーン区分", featured: false },
    { href: "#", title: "配色の基本", desc: "類似色・補色など基本的な配色の関係", featured: false },
    { href: "#", title: "色のイメージ", desc: "トーンごとの印象と感情的な効果", featured: false },
  ]

  const toolLinks = [
    { href: resolve("/approximate"), title: "色のPCCS近似", desc: "入力した色のPCCS値と慣用色名を調べる", color: "#e76f51" },
    { href: resolve("/analyze"), title: "配色の分析", desc: "PCCSの色相・トーンに基づいて配色を分析する", color: "#2a9d8f" },
    { href: resolve("/patterns"), title: "配色シミュレータ", desc: "イメージに合う色の組み合わせを実験する", color: "#6a4c93" },
  ]
</script>
...
<div class="page">
  ...
  <main>
    ...
    <header class="hero">
      ...
      <h1>
        <span class="h1-pccs">PCCS</span><span class="h1-lens"> Lens</span>
      </h1>
      ...
      <a href={contentLinks[0].href} class="featured-card">
        ...
        <div class="featured-deco" aria-hidden="true">
          <svg viewBox="0 0 120 120" width="120" height="120">
            ...
            <text x="60" y="64" text-anchor="middle" font-size="9" font-weight="700" fill="#555">PCCS</text>
          </svg>
        </div>
      </a>
...
<style>
  ...
  .blob-red    { width: 400px; height: 340px; background: #e63946; top: -80px; left: -80px; }
  .blob-yellow { width: 300px; height: 260px; background: #f4a261; top: 60px; right: 5%; border-radius: 40% 60% 30% 70% / 60% 40% 70% 30%; }
  .blob-green  { width: 360px; height: 300px; background: #2a9d8f; top: 40%; left: -60px; border-radius: 70% 30% 50% 50% / 30% 70% 40% 60%; }
  .blob-blue   { width: 280px; height: 320px; background: #457b9d; bottom: 10%; right: -40px; border-radius: 50% 50% 30% 70% / 60% 40% 70% 30%; }
  .blob-purple { width: 320px; height: 280px; background: #9b5de5; top: 55%; right: 20%; border-radius: 30% 70% 60% 40% / 50% 30% 70% 50%; }
  .blob-orange { width: 240px; height: 200px; background: #e9c46a; bottom: 5%; left: 20%; border-radius: 60% 40% 50% 50% / 40% 60% 30% 70%; }
  ...
  .h1-lens {
    background: linear-gradient(120deg, #e63946 0%, #f4a261 30%, #2a9d8f 60%, #457b9d 80%, #9b5de5 100%);
  ...
  .featured-wash {
    position: absolute;
    inset: 0;
    background: linear-gradient(135deg, rgba(230,57,70,0.04) 0%, rgba(154,93,229,0.06) 60%, rgba(69,123,157,0.04) 100%);
    pointer-events: none;
  }
