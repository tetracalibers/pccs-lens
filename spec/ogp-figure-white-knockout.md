...
- `knockoutWhite: true` のとき、手渡し PNG に ImageMagick で「背景に繋がった白」を flood-fill 透過し、透過後の外周余白を `-trim` で切り詰めて、**透過 PNG を埋め込み＆ `ogimage/data/assets/<route>/figure.png` に保存**。
...
- 処理フロー（`knockoutWhite: true` 時）：
  1. 手渡し PNG を入力に、次を実行して透過 PNG（一時ファイル）を得る：
     ...
     - `-shave 1x1`: 足した枠を除去。
     - `-trim +repage`: 透過になった外周の余白を切り詰め、`+repage` で仮想キャンバスのオフセットを消す（図版が埋め込みスロットいっぱいに収まる）。
