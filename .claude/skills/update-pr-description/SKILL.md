---
name: update-pr-description
description: 引数でPRのURLを受け取り（省略時は現在チェックアウト中のブランチに紐づくPRを対象に）、そのPRのタイトルと説明文を変更内容に合わせて更新するスキル。svxファイルの差分を確認し、draft（`draft: true`）を外して公開された記事があれば、カテゴリ別に説明文へリストアップし、記事ごとに公開後の人手作業（`visual` フラグ・OGP画像の生成・文体スタイルガイドの更新）のチェックリストを付ける。PRの説明文を整えたい・タイトルを変更内容に合わせたい・そのPRで公開した記事を一覧化したい場合に使用する。
effort: high
---

# PR説明文更新スキル

引数で受け取った GitHub PR について、**そのPRの変更内容に合わせてタイトルと説明文（本文）を更新する**ためのスキルです。

特に、`+page.svx`（記事ページ）の差分を調べ、**`draft: true` を外して公開された記事**を検出し、説明文にカテゴリ別でリストアップします。さらに、記事ごとに**公開後に人手で行う作業のチェックリスト**（`visual` フラグ・OGP画像の生成・文体スタイルガイドの更新）を付け、判定できたものはチェック済みにします。

## 入力

- **PRのURL**（任意）: `https://github.com/tetracalibers/pccs-lens/pull/<番号>` の形式。`gh` コマンドにはこの URL をそのまま渡せる。
- **URL を省略した場合**: 現在チェックアウトしているブランチに紐づく PR を対象にする。`gh pr view` / `gh pr diff` / `gh pr edit` は**引数を省略すると現在のブランチの PR** に対して動くため、URL を外して実行するだけでよい（該当ブランチに PR が無ければ `gh` が `no pull requests found for branch ...` とエラーを返す）。

以降の手順では対象 PR の指定を `$PR` と表記する。**URL が渡されたときは `$PR="<URL>"`、省略されたときは `$PR=""`（空文字＝引数なし）** として各 `gh` コマンドに渡す。

## 前提知識

### 記事の公開（draft）規約

記事ページ（`+page.svx`）の下書き・公開は frontmatter の `draft` 行で表現される。

- **下書き**: frontmatter に `draft: true` 行がある（`grades:` の下に空行を挟んで置かれる）。
- **公開**: `draft: true` 行（および直前の空行）を**削除**する。`draft` 行そのものが無くなる。

したがって「draft を外して公開された記事」とは、**そのPRの差分で `draft: true` 行が削除された `.svx` ファイル**を指す。

### `gh pr diff` の制約

- `gh pr diff <URL>` は**パス指定（`-- <path>`）を受け付けない**（`accepts at most 1 arg(s)` エラーになる）。ファイルごとに絞り込めないので、**全体の diff を1回取得し、`diff --git` セクション単位でパースする**こと。
- diff の各ファイルセクションには frontmatter の `title:` 行が文脈行（先頭スペース）または追加行（`+`）として含まれるので、そこから**日本語タイトルを抽出できる**。

### カテゴリ（ルート）と日本語見出しの対応

公開記事は `app/src/routes/<カテゴリ>/<slug>/+page.svx` に置かれる。説明文の見出しには次の日本語名を使う。

| ルート（`<カテゴリ>`） | 日本語見出し |
| --- | --- |
| `color-theory` | 色の理論 |
| `color-fields` | 色の活用分野 |
| `cg` | CGと画像処理 |

表にないカテゴリが出た場合は、`app/src/routes/+page.svelte` の該当 `title` を確認して見出しを補う。

### 公開後に人手で行う作業（チェックリストの項目）

記事を公開したら、次の作業を**人手で**行う運用になっている（README「記事の公開（draft を外したとき）」）。この3つを、公開記事ごとのチェックリストとして説明文に載せる。

| 項目 | 内容 | 判定方法 |
| --- | --- | --- |
| `visual` フラグ | 図解のあるページは frontmatter の `grades:` の次の行に `visual: true` を足す（一覧に「図解」タグが付く） | 現在の `+page.svx` に `visual: true` 行があるか。図解の有無は `$lib/demo/` の import があるか |
| OGP画像 | `/generate-ogp-image /<route>` で 1200×630 の PNG を生成する（draft のままでは glob 展開の対象外なので公開後に行う） | 記録 `ogimage/data/<route>.json` が存在するか |
| 文体スタイルガイド | `/author-style-analyzer <slug>` でその記事の「AI草稿 → 人手編集」の差分を `writing-guides/` へ反映する | `writing-guides/**/*.md` に**記事タイトル**（frontmatter の `title`）が現れるか（根拠欄に記事タイトルで書かれるため）。あくまで手がかりで、確証ではない |

- `<route>` は `app/src/routes/` を除いたパス（例: `color-theory/pccs-basics`）。`<slug>` はその末尾（例: `pccs-basics`）。
- ここでいう図解は `$lib/demo/` 配下のコンポーネントを指す。Mermaid 図だけのページに `visual: true` は付けない運用なので、Mermaid の有無は判定に使わない。
- **文体ガイドの判定は弱い**：分析しても新しいルールの根拠にならなければタイトルは載らない。記載が無い＝未分析とは限らないので、未チェックにする場合も「未分析と断定」せず、実施の要否をユーザーに確認できる形で報告する。
- PR説明文の更新（このスキル）自体は公開後作業の4つめだが、実行そのものが完了を意味するのでチェックリストには載せない。

## 手順

### 1. PRのメタ情報を取得する

```
gh pr view $PR --json number,title,body,url,baseRefName,headRefName,state
```

- URL を省略して実行し `no pull requests found for branch ...` が返った場合は、現在のブランチに PR が無い旨をユーザーに伝え、PR の URL をもらうか PR を作成してもらう。
- `state` が `OPEN` でない（`MERGED` / `CLOSED`）場合は、その旨をユーザーに伝えて、続行してよいか確認する。
- 既存の `body`（説明文）に**人手で書かれた内容がある**場合は、勝手に上書きしない。内容を確認し、置き換えてよいか／末尾に追記すべきかをユーザーに確認する。空（または本スキルが生成したと分かる定型文）なら、そのまま新しい説明文で置き換えてよい。

### 2. 変更ファイルと全体の差分を取得する

```
gh pr diff $PR --name-only          # 変更ファイル一覧
gh pr diff $PR                       # 全体の unified diff（パースに使う）
```

### 3. 公開された記事（draft を外した記事）を検出する

全体の diff を `diff --git` セクション単位でパースし、`.svx` ファイルのうち **`draft: true` 行が削除され、かつ `draft: true` 行が再追加されていない**ものを「公開された記事」とする。同じセクションの `title:` 行からタイトルを、パスからカテゴリを取り出す。

次のワンライナーがそのまま使える（`$PR` は手順 1 と同じ対象指定。URL 省略時は `gh pr diff` の引数を外す）:

```sh
gh pr diff $PR | awk '
  function flush() {
    if (file ~ /\.svx$/ && removedDraft && !readdedDraftTrue) {
      cat=file; sub(/^app\/src\/routes\//,"",cat); sub(/\/.*/,"",cat)
      printf "PUBLISHED\t%s\t%s\t%s\n", cat, title, file
    }
  }
  /^diff --git / {
    flush()
    file=$0; sub(/^diff --git a\/.* b\//,"",file)
    removedDraft=0; readdedDraftTrue=0; title=""
  }
  file ~ /\.svx$/ && $0 ~ /^[ +]title: /    { t=$0; sub(/^[ +]title: /,"",t); if(title=="") title=t }
  file ~ /\.svx$/ && $0 ~ /^-draft: *true/  { removedDraft=1 }
  file ~ /\.svx$/ && $0 ~ /^\+draft: *true/ { readdedDraftTrue=1 }
  END { flush() }
'
```

出力は `PUBLISHED<TAB>カテゴリ<TAB>タイトル<TAB>パス` の形。

**エッジケース:**

- **公開された記事が1件も無い場合**: 「公開した記事」節は作らず、手順 4〜6 で変更内容の要約だけで説明文とタイトルを整える。
- **新規追加（status `A`）で最初から `draft: true` が無い記事**: 「draft を外して」公開したわけではないため、原則リストには含めない。ただし実質的に新規公開ページなので、その存在をユーザーに伝え、「公開した記事」に含めるべきか確認する（このリポジトリでは新規ページは `draft: true` で作るのが通常なので稀）。

### 3.5. 記事ごとの公開後タスクの状況を判定する

手順 3 で検出した記事それぞれについて、前提知識「公開後に人手で行う作業」の3項目の状況を調べる。手順 3 の出力（`PUBLISHED` 行）を一時ファイル（スクラッチパッド等）に保存し、次のスクリプトを**リポジトリのルートで**実行する。

```sh
PUBLISHED_TSV=<手順3の出力を保存した一時ファイルのパス>

while IFS=$'\t' read -r _ category title file; do
  route="${file#app/src/routes/}"; route="${route%/+page.svx}"
  if [ ! -f "$file" ]; then
    printf '%s\tUNKNOWN（ローカルに該当ファイルなし）\n' "$route"; continue
  fi
  grep -qE '^visual: *true' "$file" && visual=set || visual=unset
  grep -q '\$lib/demo/' "$file" && figure=yes || figure=no
  [ -f "ogimage/data/$route.json" ] && ogp=done || ogp=todo
  grep -rqF "$title" writing-guides --include='*.md' && style=cited || style=uncited
  printf '%s\tvisual=%s\tfigure=%s\togp=%s\tstyle=%s\n' "$route" "$visual" "$figure" "$ogp" "$style"
done < "$PUBLISHED_TSV"
```

判定はローカルの作業ツリーを見るため、**対象PRのブランチをチェックアウトしていること**が前提。別ブランチのPRをURLで指定していて手元の内容が一致しない場合（`gh pr view $PR --json headRefName` と `git branch --show-current` を比べる）は、確認できない項目を `[ ]` のままにし、判定できなかった旨をユーザーに報告する（推測でチェックを付けない）。

### 4. その他の主な変更を要約する

説明文の「その他の主な変更」節と、新しいタイトルを決めるために、PR全体の変更を把握する。

```
gh pr view $PR --json commits --jq '.commits[].messageHeadline'
```

- コミット件名と変更ファイル一覧（手順 2）から、公開記事以外の主な変更を日本語で数点にまとめる（例: 新規ページ雛形・下書きの追加、既存ページの加筆、ヘッダーナビの調整、ドキュメント・スキル・スタイルガイドの更新など）。
- 細かなコミット（`[skip ci]` の規約追記や軽微な修正）は個別に列挙せず、まとめて要約する。

### 5. 説明文（本文）を組み立てる

下記の雛形（PR #87 の説明文がベース）に沿って Markdown を組み立てる。公開記事はカテゴリごとに `###` でまとめ、`- **タイトル** — パス` の形で列挙し、**記事ごとに公開後タスクのチェックリストをネストさせる**。

```markdown
## 公開した記事（draft を外して公開）

このブランチで下書き（`draft: true`）を外して公開した記事は次の <件数> 件です。
各記事の公開後タスク（人手作業）の状況を併記しています。

### <日本語カテゴリ名（例: 色の理論）>

- **<記事タイトル>** — `app/src/routes/<カテゴリ>/<slug>/`
  - [x] `visual: true` を追加済み（図解あり）
  - [ ] OGP画像を生成 — `/generate-ogp-image /<カテゴリ>/<slug>`
  - [ ] 文体スタイルガイドを更新 — `/author-style-analyzer <slug>`
- **<記事タイトル>** — `app/src/routes/<カテゴリ>/<slug>/`
  - [x] `visual` フラグ不要（図解なし）
  - [x] OGP画像を生成済み
  - [x] 文体スタイルガイドへ反映済み

### <日本語カテゴリ名（例: 色の活用分野）>

- ...

## その他の主な変更

- <変更点1>
- <変更点2>
- ...
```

- パスは `+page.svx` を除いたディレクトリまで（`.../<slug>/`）を `code` で示す。CG記事はユニットが挟まって1階層深い（`app/src/routes/cg/<unit>/<slug>/`）ので、`route` もそれに合わせる（例: `cg/basics/anti-aliasing`）。
- 公開記事が無い場合は「公開した記事」節を省き、「その他の主な変更」（または「変更内容」）節のみとする。
- カテゴリの並び順・記事の並び順は、`app/src/routes/+page.svelte` のカテゴリ順や自然な流れに合わせて読みやすく整える。

#### チェックリストの書き分け

手順 3.5 の判定結果に応じて、記事ごとに `visual`・OGP・文体ガイドの行を1本ずつ書く（判定できたものは `[x]` にしておく）。左が判定、右がそのまま書く行。

```
figure=yes, visual=set    →  - [x] `visual: true` を追加済み（図解あり）
figure=yes, visual=unset  →  - [ ] `visual: true` を追加（図解あり）
figure=no                 →  - [x] `visual` フラグ不要（図解なし）

ogp=done                  →  - [x] OGP画像を生成済み
ogp=todo                  →  - [ ] OGP画像を生成 — `/generate-ogp-image /<route>`

style=cited               →  - [x] 文体スタイルガイドへ反映済み
style=uncited             →  - [ ] 文体スタイルガイドを更新 — `/author-style-analyzer <slug>`

UNKNOWN                   →  3項目とも「- [ ] …（未確認）」にし、報告で理由を伝える
```

- 未チェックの項目が1つでも残る場合は、リストの直後に案内を1行添える: `未チェックの項目は公開後の人手作業です（README「記事の公開（draft を外したとき）」を参照）。`
- すべてチェック済みなら案内行は不要。
- 文体ガイドが複数記事とも `style=uncited` なら、案内行に**まとめて実行できる**旨を添えてよい: `文体ガイドはカンマ区切りでまとめて分析できます（/author-style-analyzer <slug1>,<slug2>）。`

### 6. タイトルを決める

そのPRの変更内容全体を端的に表す日本語タイトルにする。公開記事がある場合はそれが主眼になることが多い。

- 例: `色の理論・活用分野の記事を追加し、4記事を公開`
- コミット件名やブランチ名を機械的に流用せず、**PR全体の成果**が伝わる要約にする。

### 7. PRのタイトルと説明文を反映する

説明文は改行・Markdown を保つため、一時ファイル（スクラッチパッド等）に書き出してから `--body-file` で渡す。

```
gh pr edit $PR --title "<新しいタイトル>" --body-file <一時ファイルのパス>
```

反映後、`gh pr view $PR --json title,url --jq '.title, .url'` で結果を確認する。

### 8. ユーザーに報告する

- 検出した「公開された記事」の一覧（タイトル・カテゴリ・パス）。
- 変更前後の PR タイトル。
- 説明文に記載した「その他の主な変更」の要点。
- **未チェックのまま残った公開後タスク**（どの記事の `visual` フラグ／OGP画像／文体ガイドが未対応か）。1件でもあれば、対応するコマンド（`/generate-ogp-image /<route>`・`/author-style-analyzer <slug>` など）を添えて促す。判定不能（`UNKNOWN`）があればその理由も伝える。
- 文体ガイドの項目は判定が弱い（前提知識参照）ため、未チェックにしたものは「`writing-guides/` に記事タイトルの記載が無かった」という事実として伝え、すでに分析済みかどうかをユーザーに確認する。
- タイトルは要約であり、意図に合わせて調整可能である旨を添える。

## 注意事項

- **勝手に既存の説明文を破棄しない。** 人手で書かれた本文がある場合は、置き換え/追記の方針をユーザーに確認する（手順 1）。
- 公開判定は必ず**svx の差分の `draft: true` 行削除**で行う。YAML（`content-pages/*.yaml`）の `DraftLink`→`PageLink` 置換だけでは、記事本文の公開状態を正しく判定できないので根拠にしない。
- `gh pr diff` はパス指定不可。全体 diff をパースすること（手順 2・3）。
- **チェックリストのチェックを後退させない。** 再実行時、既存の説明文で `[x]` になっている項目を、手順 3.5 の判定結果だけを根拠に `[ ]` へ戻さない（人手でチェックした可能性がある）。判定と食い違う場合は、その項目を挙げてユーザーに確認する。
- **チェックリストの項目を勝手に増やさない。** 載せるのは `visual` フラグ・OGP画像・文体スタイルガイドの3項目のみ。運用手順が変わったときは README「記事の公開（draft を外したとき）」と本スキルの両方を更新する。
- URL は `gh` の各サブコマンドにそのまま渡せる。**URL を省略した場合は現在チェックアウト中のブランチの PR が対象**になる（手順 1）。フォーク由来の PR ではヘッドが別リポジトリになる点に注意（`gh pr diff/view/edit` は URL 指定でも現在ブランチ経由でも動く）。
- 本スキルはコード変更を伴わない（PR のメタ情報更新のみ）ため、`npm run check` は不要。
