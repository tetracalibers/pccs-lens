...
### 5. タスクリストを YAML に追随させる

`CgDraftLink` を `PageLink` に置き換えると、OGP・文体解析の 2 つのタスクリストに残った `- [ページ未作成] <タイトル>` の行が実態とずれる。照合スクリプトで揃える（リポジトリルートで実行）。
...
- 更新されるのは `ogimage/OGP-TASKLIST.md` と `writing-guides/STYLE-ANALYSIS-TASKLIST.md`。今回作ったページの行が `- [ページ未作成] <タイトル>` から ``- [draft] `/cg/<ユニット>/<slug>` `` に変わる（OGP はユニット単位、文体解析はセクション単位で掲載位置が異なるが、どちらもスクリプトが面倒を見る）。
