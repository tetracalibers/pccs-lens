...
- 報告の最後に、本文の草稿執筆へ進むための `write-cg-draft` スキルの呼び出し方を、**手順 2 で決めた実際の slug を埋め込んで**明記する。例（`<slug>` は具体的な文字列に置き換える）:

  ```
  /write-cg-draft <slug>
  ```

  たとえば slug が `image-digitization` なら `/write-cg-draft image-digitization` と案内する。
