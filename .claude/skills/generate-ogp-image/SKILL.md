...
- **既存記録に図版があれば流用する**（手順 0）。保存済みアセット `ogimage/data/assets/<route>/figure.<ext>` が実在すれば、その永続パスを `figure` に指定して再利用する（差し替え時のみ新規手渡しを受け取る）。
...
- 記録・図版アセット・出力 PNG は**コミット対象**。生成後にコミットに含める。
