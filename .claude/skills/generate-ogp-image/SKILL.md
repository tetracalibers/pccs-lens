...
description: 引数で受け取ったページスラッグ（glob 可）の OGP 画像を手動生成するスキル。ogimage/ の正典設定でバリエーションを判定し、対象ページのコードからタイトル・パンくずを解決し、長いタイトルの改行案を提示・確認し、図版は第2引数のパスで受け取り（nested-fig 対象で未指定のときのみ要否を対話確認）、第3引数に all / background が渡されたら白背景の判定も透過の確認もせずその透過モードで生成し、確定値を JSON で描画スクリプト（ogimage/render.mjs）に渡して PNG を app/static/ogp/ に生成し、マニフェストを更新する。OGP画像を作りたい・生成したい場合に使用する。
effort: high
---
...
- **白背景の透過モード**（第3引数・省略可）: `all` または `background`。**渡された時点で「白背景 PNG を透過する」という指定**として扱う。
  ...
  - 位置引数だが値が一意なので、**`all` / `background` に一致する引数は第2引数の有無に関わらず透過モード指定として解釈**し、それ以外の非スラッグ引数を図版パスとして解釈する。
...
**第3引数で透過モード（`all` / `background`）を受け取っていれば、この節は丸ごとスキップする。** 白背景かどうかの画像解析も、要否・モードの確認もせず、`knockoutWhite: true` ＋ 受け取ったモード（`background` のときは `knockoutMode` を省略してよい）で手順 6 に進む。glob で複数ルートを処理していて各ルートに新規図版を手渡しで受け取る場合は、**その全ての新規図版に同じモードを適用**する。ただし図版が PNG 以外のとき・既存記録の図版を流用するときは適用せず、無視した旨を伝える（「入力」参照）。
...
- glob で複数ルートをそれぞれ個別の図版で処理した場合は、各ソースについて同じ判定を行う。
...
- **タスクリストを更新する**（スキル層の責務。描画スクリプトはこれを行わない）：生成に成功したルートについて、`ogimage/OGP-TASKLIST.md` の該当行を**チェック済み `- [x] \`/<route>\`` に更新する**。該当行は現在の表記により次のいずれか。いずれの形でも最終的に `- [x] \`/<route>\`` にそろえる。
  ...
  - 既に `[x]` の場合はそのままにする。
  - glob で複数生成したときは、成功した全該当行に同じ更新を行う。
