...
1. 引数の先頭トークンが `app/src/lib/demo/` 配下のディレクトリ名として妥当なもの（英字の kebab-case。例: `munsell`, `color-wheel`）かどうかを見る。
   - 先頭が日本語の説明文で始まる場合 → **パターンA**
   - 先頭が kebab-case のディレクトリ名の場合 → **パターンB または C**（先頭トークンが `<配置先ディレクトリ>`）
2. パターンB / C の切り分け：`<配置先ディレクトリ>` に続く残りの引数の先頭部分が、既存の `TODO: <図のタイトル>` マーカーのいずれかと一致するかを調べる。マーカー一覧は次で取得する。

   ```bash
   grep -rn "TODO:" app/src/routes/color-theory app/src/routes/color-fields
   ```

   - 一致するものがある → **パターンC**（一致した部分が `<図のタイトル>`、残りが `<図の内容>`）
   - 一致するものがない → **パターンB**（残り全体が `<図の内容>`）
...
### パターンC：`/svg-diagram-component <配置先ディレクトリ> <図のタイトル> <図の内容>`

- `<配置先ディレクトリ>` の扱いはパターンBと同じ（`app/src/lib/demo` 配下にコンポーネントを作成する）
- `<図のタイトル>` が指定されているため、`app/src/routes/color-theory`・`app/src/routes/color-fields` 配下から、`TODO: <図のタイトル>` と書かれているページを探す
- コンポーネントを作成したら、対象ページに対して次を行う：
  1. `<script>` ブロックに、作成したコンポーネントの import 文を追加する（`SVGWrapper` が未 import であればあわせて追加する）
  2. 本文中の `TODO: <図のタイトル>` を、`<SVGWrapper>` で包んだコンポーネントの使用箇所に置き換える（例: `<SVGWrapper>\n  <EmergencyExitSign />\n</SVGWrapper>`）
