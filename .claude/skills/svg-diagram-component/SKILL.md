...
- **`<図の指定>`（必須）** … 作りたい図の名前・内容。
  ...
  **`:::Todo{type="text"}` は図版のプレースホルダではない。** `type="text"` は文章の加筆メモ（`author-style-writer` の担当）なので、このスキルの対象から外す。図版のプレースホルダは属性なし、または `{type="figure"}` の `:::Todo` だけ。
...
- **ヒットした箇所が `:::Todo{type="text"}` ブロックの中なら、プレースホルダとして扱わない。** 文章の加筆メモなので、図版の依頼としては見つからなかったものとみなす（パターンA／Bへ倒す）。ヒット行の直前の `:::Todo` 開始行に `{type="text"}` が付いていないかを必ず確かめる。

  ```bash
  # ヒットしたページで、図版のTODO（属性なし・figure）だけを見る
  grep -n ':::Todo' <ヒットしたページ> | grep -v 'type="text"'
  ```
