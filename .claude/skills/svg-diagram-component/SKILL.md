...
- **`<図の指定>`（必須）** … 作りたい図の名前・内容。
  ...
  **`:::Add` / `:::Delete` / `:::Todo{type="fix"}` は図版のプレースホルダではない。** `:::Add` は文章の加筆メモ、`:::Delete` は削除予定の記述（どちらも `author-style-writer` の担当）、`type="fix"` は記述を直したいメモなので、いずれもこのスキルの対象から外す。図版のプレースホルダは属性なし、または `{type="figure"}` の `:::Todo` だけ。
...
- **ヒットした箇所が `:::Add` / `:::Delete` / `:::Todo{type="fix"}` ブロックの中なら、プレースホルダとして扱わない。** 文章の加筆メモ・削除予定の記述・記述の修正メモなので、図版の依頼としては見つからなかったものとみなす（パターンA／Bへ倒す）。ヒット行の直前のブロック開始行が `:::Todo`（属性なし・`{type="figure"}`）であることを必ず確かめる。

  ```bash
  # ヒットしたページで、図版のTODO（属性なし・figure）だけを見る
  grep -n ':::Todo' <ヒットしたページ> | grep -v 'type="fix"'
  ```
