...
1. `<TODOから始まる文>` に対応するプレースホルダを持つページを `app/src/routes/color-theory`・`app/src/routes/color-fields` 配下から探す。

   図版のプレースホルダは `:::Todo` ディレクティブで書くのが現行の規約（`writing-guides/syntax-guide.md` ルール4）で、この場合 `TODO` の語はディレクティブ名の側にあり、図の説明だけが本文行になる。
   ...
   そのため検索は、引数から先頭の `TODO`・`TODO:`・`TODO：` を取り除いた**説明部分**で行う。

   ```bash
   # 例: 引数が「TODO：水晶体分光透過率のグラフ」なら
   grep -rn "水晶体分光透過率のグラフ" app/src/routes/color-theory app/src/routes/color-fields
   ```

   規約導入前に書かれたページには、HTMLコメント（`<!-- TODO: 〜の図解 -->`）や地の文の `TODO：〜` の形も残っている。説明部分での検索はこの3形式のいずれにもヒットするので、見つかった形に応じて手順3の置き換え範囲を決める。

   引数内での `<TODOから始まる文>` の範囲は、ページ内で見つかったプレースホルダの説明部分と逐語一致する部分に、先頭の `TODO`（＋コロン）を加えたものとする。その手前にある kebab-case トークンが `<配置先ディレクトリ>`、後に続く残りが `<図の内容>` にあたる。
2. コンポーネントを作成する。
