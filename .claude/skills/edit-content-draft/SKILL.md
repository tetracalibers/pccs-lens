...
3. 引数で与えられた変更指示だけでは**どのように変更するかの方向性が不明瞭**な場合（例: 「この節を直して」「ここを良くして」だけで、どの方向に変えたいのかが指示から読み取れない等）は、質問して方向性を確定してから編集する。
4. ただし、方向性が**スタイルガイド（`.claude/skills/write-content-draft/style-guide.md`）の規約で定まる**内容であれば、質問せずにスタイルガイドに沿って編集する。たとえば「導入文を見直して」という指示は、導入文のテイストがスタイルガイドの「記事の冒頭（導入文）」に記事の種類ごとに規定されているため、確認せずにその規約に従って編集する。
