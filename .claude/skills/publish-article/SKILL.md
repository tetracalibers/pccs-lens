...
**公開時タスクより先に必ず行う。** `:::Todo` / `:::Add` / `:::Delete` / `:::Edit` / `:::Modify` / `:::Pending` は読者に見える形で表示されるため、残したまま公開してはいけない（`writing-guides/syntax-guide.md` ルール4）。`{fixme}` の付いた `:::Action`（AIが書いたまま人手が入っていないデモ誘導文）も同じ扱いで止める。

```sh
grep -nE '^:{3,}(Todo|Add|Delete|Edit|Modify|Pending)' <対象の .svx>
...
| `:::Add` / `:::Delete` / `:::Edit` / `:::Modify` | `/apply-edit-requests <記事slug>` — **何件あっても1行**（一括で対応するスキルのため） |
...
- すでに `` - [ ] `/<route>` `` になっている、または `- [x]`（分析済み）・`- [~]`（分析済みだが改稿された＝再分析待ち）の場合は何もしない。**`[~]` を `[ ]` に落とさない**（再分析待ちと未分析は別。→ `commit-this` 手順 1.8 b）
...
- **未分析記事の残件**: 手順2で `writing-guides/STYLE-ANALYSIS-TASKLIST.md` を書き換えたら、更新後のファイルを数え直し、未分析記事の**件数とスラッグの全列挙**を示す。今回公開した記事が加わって残件が増えるので、次にどこまで `/author-style-analyzer` にかけるかの判断材料になる
  ...
  grep -n '^- \[~\]' writing-guides/STYLE-ANALYSIS-TASKLIST.md   # 再分析待ちのスラッグ
  ```

  - 数えるのは `- [ ]` の行だけ。`[draft]`・`[ページ未作成]` は分析対象外なので、件数にもスラッグ一覧にも含めない
  - **`[~]`（再分析待ち）は未分析の件数に混ぜず、別枠として件数とスラッグを示す**。一度も分析していない記事と、分析後に改稿されて再分析の候補になった記事は別物（→ `commit-this` 手順 1.8 b）。0件なら触れなくてよい
