...
- 未チェック `[ ]` … まだ実行していない、または**実行後に本文が書き換わってチェックが外れた**
...
- **`[x]` は「そのときの本文」に対する記録で、本文が書き換わると無効になる。** 非 draft（公開済み）の記事の本文を変更したコミットでは、`commit-this`（手順 1.8）が該当行を `[x]` から `[ ]` に戻す。記法の統一（記事全体・文単位・ブロック数式の説明文）は本文に依存するため、加筆・修正のあとは改めて `/format-math-notation <slug>` を回す。
  - **下書き（`draft: true`）の記事では外さない。** 執筆中は本文が動き続けるので、編集ごとに外しても意味がない。
  - **frontmatter だけの変更（`visual` フラグの追加、`draft` の付け外し）では外さない。** 記法は変わらない。
  - **`format-math-notation` を回して `[x]` を付けたコミット自身では外さない。** 記法の整備そのものが本文を書き換えるため。
