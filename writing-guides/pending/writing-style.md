...
- WS-P108｜デモの提示単位を「:::Action（着眼点）→ デモ本体 → :::Foldable{title="Three.jsによる実装概要"}（実装コード）」の三点セットで閉じ、実装コードはデモの後ろの折りたたみへ置いて解釈文を添えない：3記事のいずれもデモを持つCG記事で、WS-064／WS-065 がデモ手前の順序までを規定するのに対し、デモ後の実装コードの位置づけを扱うIDが無い。ただし折りたたみの掲載自体は add-threejs-demo スキルの規約側で決まっており、著者の構成判断と媒体規約を切り分けられない｜支持: /cg/modeling/curve-surface-equations, /cg/modeling/bezier-curve-surface, /cg/modeling/bezier-curve-properties
