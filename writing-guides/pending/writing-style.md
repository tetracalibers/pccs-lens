...
支持記事が同一シリーズ・同一テーマ・近接時期に偏る項目は、記事数が閾値を超えても他タイプ・他ドメインでの再現が確認できるまで昇格させない。ソース上の改行・記法（`:Anki`・`:WithGradeTag`・バッククォート・一文一行）は `syntax-guide.md`・`math-notation-guide.md` の管轄で、構成上の癖としては扱わない。
...
- WS-P091｜手法の利点と限界を「〈手法〉の強み：〜」「〈手法〉の弱点：〜」という対の `###` 小節へ分け、強み→弱点の順に連続配置する：1記事のみで、同記事内のもう一方の手法は対の小節に分かれず1小節内の段落で処理されており、2手法の扱いが対称でない（節末に引き換えを添えること自体は本体 WS-066）｜支持: /cg/transformation/perspective-and-parallel-projection
- WS-P092｜2手法を対比した記事の末尾を、使い分けを述べる2文と枠外へ送る `:::Note` で閉じる：1記事のみで、同シリーズの直前記事は一段抽象化した凝縮文1文で閉じており締めの担体が揃わない（総括節の不在自体は本体 WS-004）｜支持: /cg/transformation/perspective-and-parallel-projection
- WS-P093｜混同しやすい決定要因の否定→特定を、本文中の切り出し箇所ではなく記事末尾の `:::Note` に置く：1記事1箇所のみで、記事全体の枠組みから生じる誤解を末尾で潰す配置が型なのか単発なのか判別できない（否定→特定そのものは本体 WS-043）｜支持: /cg/transformation/perspective-and-parallel-projection
...
- WS-P106｜デモの直後に :::Foldable でThree.jsの実装コードを畳んで置き、本文の説明はコードを挟まずその後ろで続ける：`add-threejs-demo` スキルがCG記事共通の掲載規約としてデモと実装コードの併載を定めており、著者が選び取った構成ではなくツール側のテンプレートに従った結果である可能性が高い｜支持: /cg/transformation/viewing-pipeline-transformations, /cg/transformation/parallel-projection-types, /cg/modeling/shape-model-overview, /cg/rendering/photorealism-and-reality-elements
- WS-P107｜デモを節の終端にせず、デモ（と畳んだ実装コード）の後で地の文へ戻り、その手法の限界または確認できた一般命題を述べて節を閉じる：2記事で確認できるが、閉じ方が「できないこと」（次節の動機）と「確認できたことの一般化」に割れ、どちらを置くかの条件を特定できない。デモ直後の読み取り一文は本体 WS-063、デモに至るまでの順序は WS-064・WS-065 が担当｜支持: /cg/modeling/shape-model-overview, /cg/rendering/photorealism-and-reality-elements
- WS-P108｜デモの提示単位を「:::Action（着眼点）→ デモ本体 → :::Foldable{title="Three.jsによる実装概要"}（実装コード）」の三点セットで閉じ、実装コードはデモの後ろの折りたたみへ置いて解釈文を添えない：デモ後の実装コードの位置（デモの後ろの畳んだブロック・本文の説明順に数えない）は本体 WS-065 が既に規定しており、残差は「実装コードに解釈文を添えない」の一点にとどまる。その折りたたみ掲載自体が add-threejs-demo スキルの掲載規約（:::Action → デモ → :::Foldable の固定テンプレート）で決まっているため、著者の構成判断と媒体規約を切り分けられない。支持3記事はいずれも数式主体のCG・モデリング記事に閉じる｜支持: /cg/modeling/curve-surface-equations, /cg/modeling/bezier-curve-surface, /cg/modeling/bezier-curve-properties
- WS-P109｜式の根拠（導出・前提知識・記号選択の理由）を :::Note へ退避し、本文の主線は結果の式とその読み方だけで進める：3記事で確認できるが、WS-068（本筋から外れる補足知識の退避）との差は「退避される中身が本筋の式の妥当性そのものである」という一点で、退避先の判断としては同じ操作に見える。WS-068 の適用条件を広げるべきか別ルールとして立てるべきかを決められない。支持3記事はいずれも数式主体のCG記事に閉じる｜支持: /cg/modeling/curve-surface-equations, /cg/modeling/quadratic-curve, /cg/modeling/bezier-curve-properties
- WS-P110｜同一の下位項目集合を、離れた複数の上位節で同じ順序のまま反復し、記事を項目×観点のマトリクス構造にする（別文脈での再列挙による継承の明示を含む）：2記事のみで、しかも実現形が異なる（一方は並列節でのH3反復、他方はリード予告と後続節での同順再掲）。WS-031 は予告した箇条書きと直後の小節の一段対応で別物だが、単一ルールとして立てるには事例数と構造の一致が足りない｜支持: /cg/modeling/quadratic-curve, /cg/modeling/bezier-curve-properties
