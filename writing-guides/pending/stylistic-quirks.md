...
`author-style-analyzer` が分析のたびにこのファイルを読み、根拠が増えた項目を `stylistic-quirks.md` の主要ルールへ**昇格**する／新たな保留を**追記**する対象とする。

**記事が増えても解けない理由で再審査を打ち切った項目は、ここではなく `../rejected/stylistic-quirks.md` へ移す**（`node scripts/style-pending-promote.mjs` が棄却IDを昇格候補から外す）。棄却にできるのは媒体規約・一般技法・既存ルール・観点違い・型不収束の5区分だけで、**単一シリーズ閉塞・支持記事数の不足はここに残す**（別シリーズの記事が増えれば解けるため。棄却へ流すと再現の機会が来ても見に行かない死蔵になる）。
