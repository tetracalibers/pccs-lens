...
- TF-P093｜隣接概念の違いを、同じ対象に同じ操作を与えた対比デモの「結果の差」として体感させる（左右並置や効きを`0`にしたときの同一化を含む）：Three.js デモを持つ2記事でのみ確認でき、対比構図が `add-threejs-demo` スキルによるデモ設計の結果か著者の説明判断かを完成記事から切り分けられない。デモを添える対象の選別は本体 TF-056、着眼点の指示は TF-057、観察者としての動員は TF-058 が担当｜支持: /cg/modeling/shape-model-overview, /cg/rendering/photorealism-and-reality-elements
- TF-P094｜同一対象について並存する2つの表現形式（標準形とパラメータ形式）の間を往復して一方から他方を導き直し、読者が既に知っている恒等式への一致をもって「表現間の橋」を架け、導いた関係式と元の式を TermCard で並置する：1記事のみ。TF-041（既習結果＋増分・特殊化での整合確認）・TF-017（同じ定義の言い換え）とは方向が違うが、複数形式を併記する題材（2次曲線・曲線曲面の数式表現）に固有の運びである可能性を切り分けられない｜支持: /cg/modeling/quadratic-curve
- TF-P095｜式の理由づけを逆向きに行う。図形が満たすべき性質（線対称性など）を先に言い切り、その要請から式がその形でなければならないことを導いて、同じ要請が別の表現形式でどう現れるかの指摘で閉じる：1記事のみ。TF-008・TF-038・TF-042 はいずれも与えられた式へ後から意味・理由を添える方向で、要請先出しの逆向きは既存IDに無い。反証では「唯一明快な例」と評価されたが、他記事での再現が示せていない｜支持: /cg/modeling/quadratic-curve
