...
### ルール4: あとで手を入れたい箇所は `:::Todo` / `:::Add` / `:::Delete` / `:::Fix` / `:::Pending` で明示する
...
- **書いたものの、採用するか決めきれていない文章は `:::Pending` で囲みます**。無彩色の帯と行頭の `?`（ndiff がヒント行に使う記号）で表示されます。
- 緑（足す）・赤（消す）・琥珀（直す）に対して**無彩色**なのは、採用も削除もまだ決まっていない状態を表すためです。
...
- 決着したら、**採用ならブロックだけを外して本文に馴染ませ、不採用なら囲んだ記述ごと削除します**。
...
記事を公開する（`draft: true` を外す）時点で `:::Todo` / `:::Add` / `:::Delete` / `:::Fix` / `:::Pending` が残っていないか確認します。読者に見える形で残るため、**残っている場合は自分で判断せず、対応するかブロックを外すかをユーザーに確認してください。**
...
| `:::Pending` | 書いたが採用するか決めきれていない文章（→ ルール4） |
