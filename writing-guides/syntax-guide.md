...
- 属性を省略したときの既定がこちらです（`{type="figure"}` と書いても同じ）。ブロックには「TODO：図解.1」のように、ページ内の図解TODOの通し番号が表示されます。
...
- **文章の加筆には必ず `{type="text"}` を付けます**。ブロックには「TODO：追記」と表示され、図解の番号にも数えられません。
...
| `:::Todo` / `:::Todo{type="text"}` | 図版・デモの不足（既定）や加筆したい箇所（`type="text"`）など、あとで手を入れたい箇所のメモ（→ ルール4） |
