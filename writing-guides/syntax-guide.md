...
| 形 | 記法 | 例 |
| --- | --- | --- |
| テキスト（地の文に埋める） | `:Name[本文]{属性}` | `:Mark[画素]` |
...
```
色相が`2`（赤）で、トーンが`v`（vivid）の色は:Mark[v2]と表します。
```

`:Mark[]` の中ではインラインコードを使えないため、両方に当てはまる表記（上の例の `v2`）は `:Mark[]` を優先し、バッククォートで囲みません。
...
なお、`:Mark[]` の中ではインラインコードを使えないため、`:Mark[]` で囲む語に数字が含まれる場合は、その数字をバッククォートで囲まずそのまま書きます（例: `:Mark[10YR]`、`:Mark[n色覚]`）。
...
`guide-content` レイアウトで使えるディレクティブは次のとおりです。**草稿段階では `:Mark[]` / `:::Note` / `:::Example` を中心に**使い、以下は必要なときだけ使います。
...
- `title="..."` … カードの見出し。
- `ankiTitle="hide"`（既定）/ `"mark"` / `"show"` … 暗記モードでのタイトルの扱い。`hide` はタイトルを伏せ字にし、`mark` は `:Mark[]` と同じ扱い（通常表示でも強調され、暗記モードで隠れる）、`show` は隠しません。カード自体が「用語を当てる問題」になるときは `hide` か `mark`、単なる見出しなら `show` にします。
