...
2. **草稿の執筆**：`/write-content-draft <slug>` で、インタビューを通じて集めた情報をもとに本文の草稿を執筆する（3種類すべてに対応。第二引数に含めたいキーワードをカンマ区切りで渡せる）。
3. **編集・手直し**：`/edit-content-draft <slug> <編集指示>` で、すでに本文のあるページに編集を加える（執筆スタイルガイドに従い、前後の文章・セクションの流れも整える）。
